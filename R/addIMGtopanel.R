...
addIMGtopanel <- function(p1, img.list, pal = colorRamps::matlab.like(100)) {
  ...
  rows <- unlist(lapply(stringr::str_split(panel.vp, "[.]"), function(x) {
    stringr::str_split(x[2], "-")[[1]][1]
  }))
  rows <- as.numeric(rows)
  rows <- match(rows, sort(unique(rows)))
  cols <- unlist(lapply(stringr::str_split(panel.vp, "[.]"), function(x) {
    stringr::str_split(x[2], "-")[[1]][2]
  }))
